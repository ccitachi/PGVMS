...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        for i in range(n_blocks):
            setattr(self, 'UpBlock1_' + str(i+1), ResnetPGSNBlock(ngf * mult, use_bias=True))
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                model_decoder += [weight_norm(nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                ...
                model_decoder += [Upsample(ngf * mult),
        ...
        model_decoder += [weight_norm(nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0))]
        model_decoder += [nn.Tanh()]

        self.model = nn.Sequential(*model)
        self.model_decoder = nn.Sequential(*model_decoder)
    def forward(self, input, layers=[], image_label=0, device=0, encode_only=False):
        if -1 in layers:
        ...
        if len(layers) > 0:
            ...
            for i in range(self.n_blocks):
                fake = getattr(self, 'UpBlock1_' + str(i+1))(fake,image_label.data)

            fake = self.model_decoder(fake)
            return fake
...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        attn_scores = torch.matmul(Q, K.transpose(-2, -1)) / (self.embed_dim ** 0.5)  # (batch_size, seq_len_q, seq_len_k)

        if mask is not None:
            attn_scores = attn_scores.masked_fill(mask == 0, float('-inf'))

        attn_probs = F.softmax(attn_scores, dim=-1)  # (batch_size, seq_len_q, seq_len_k)
        attn_probs = self.attn_dropout(attn_probs)

        # Attention output
        attn_output = torch.matmul(attn_probs, V)  # (batch_size, seq_len_q, embed_dim)

        # Apply the final output projection
        output = self.output_proj(attn_output)

        # Return the residual connection
        return self.norm(output + query)  # (batch_size, seq_len_q, embed_dim)


class ImageTextCrossAttention(nn.Module):
    def __init__(self, img_embed_dim, text_embed_dim=512, num_heads=8, dropout=0.1):
        super(ImageTextCrossAttention, self).__init__()

        # Assume images are embedded into img_embed_dim
        # Assume text embeddings (e.g., from a transformer model) have dimension text_embed_dim

        # Projection to align both image and text embeddings into the same space
        self.img_proj_1 = nn.Linear(text_embed_dim, img_embed_dim)  # Align image embedding to text embedding space
        self.img_proj_2 = nn.Linear(text_embed_dim, img_embed_dim)
        # Cross-attention module
        self.cross_attention = CrossAttention(embed_dim=img_embed_dim, num_heads=num_heads, dropout=dropout)

    def forward(self, image_feats, text_feats, mask=None):
        # image_feats: (batch_size, seq_len_img, img_embed_dim)
        # text_feats: (batch_size, seq_len_txt, text_embed_dim)

        # Project image features to the text embedding space (image as query, text as key/value)
        query = image_feats  # (batch_size, seq_len_img, text_embed_dim)
        
        # Text features are already in the desired embedding space, use them directly as key and value
        key = self.img_proj_1(text_feats)  # (batch_size, seq_len_txt, text_embed_dim)
        value = self.img_proj_2(text_feats)  # (batch_size, seq_len_txt, text_embed_dim)

        # Apply cross-attention
        output = self.cross_attention(query, key, value, mask)

        return output  # (batch_size, seq_len_img, text_embed_dim)
    
class ResnetPGSNBlock(nn.Module):
    def __init__(self, dim, use_bias):
        super(ResnetPGSNBlock, self).__init__()
        self.pad1 = nn.ReflectionPad2d(1)
        self.conv1 = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=0, bias=use_bias)
        self.conv1_2 = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, bias=use_bias)
        self.conv1_3 = nn.Conv2d(2*dim, 2*dim, kernel_size=1, stride=1, padding=0, bias=use_bias)
        self.conv1_4 = nn.Conv2d(2*dim, 2*dim, kernel_size=1, stride=1, padding=0, bias=use_bias)
        self.norm1 = PGSN(dim)
        self.relu1 = nn.ReLU(True)
        self.maxpooling = F.adaptive_max_pool2d
        self.avgpooling = F.adaptive_avg_pool2d
        self.pad2 = nn.ReflectionPad2d(1)
        self.conv2 = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=0, bias=use_bias)
        self.norm2 = PGSN(dim)

    def forward(self, x, label,device=0):
        out = self.pad1(x)
        out_bias = self.conv1_2(out)
        out_bias = torch.cat([self.maxpooling(out_bias, output_size=(1, 1)),self.avgpooling(out_bias, output_size=(1, 1))],dim=1 )
        out_bias = self.conv1_3(out_bias)
        out_bias = self.relu1(out_bias)
        out_bias = self.conv1_4(out_bias)
        out_bias = out_bias.squeeze(-1).squeeze(-1)
        
        label = label + out_bias
        out = self.conv1(out)
        out = self.norm1(out, label,out_bias,device)
        out = self.relu1(out)
        out = self.pad2(out)
        out = self.conv2(out)
        out = self.norm2(out, label,out_bias,device)

        return out+x
class PGSN(nn.Module):#Prompt Guide Style Normalization (PGSN)
    def __init__(self, num_features, eps=1e-5):
        super(PGSN, self).__init__()
        self.eps = eps
        ...
        self.conv1x1_gamma = nn.Conv2d(512,num_features,1,1,0)
        self.conv1x1_beta = nn.Conv2d(512,num_features ,1,1,0)
        self.conv1x1_bias = nn.Conv2d(512,num_features ,1,1,0)
        # self.conv1x1 = nn.Conv2d(num_features,num_features *2 ,1,1,1)
    ...
    def forward(self, input,label,bias,device=0):

        newlabel = label.unsqueeze(-1).unsqueeze(-1)
        newbias = bias.unsqueeze(-1).unsqueeze(-1)

        new_bias = self.conv1x1_bias(newbias)
        gamma = self.conv1x1_gamma(newlabel) + new_bias
        beta = self.conv1x1_beta(newlabel) + new_bias
