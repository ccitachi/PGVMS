...
from util.losses import bayeGen_loss
class MLPA_LOSS(nn.Module):
    def __init__(self):
        ...
        self.bayeGen_loss = bayeGen_loss
    ...
    def forward(self,inputs,targets):
        ...
        # MLPA_avg = self.bayeGen_loss(inputs_OD/(inputs.shape[2]*inputs.shape[3]),targets_OD/(inputs.shape[2]*inputs.shape[3]))
        ...
        # # loss_MLPA += torch.sum(torch.where((inputs_OD - targets_OD >= targets_OD * -0.1) & (inputs_OD - targets_OD <= targets_OD * 0.1), MLPA_histo, MLPA_avg + MLPA_histo))
        # loss_MLPA  += MLPA_avg + MLPA_histo + MLPA_block
    ...
    def compute_OD(self,image):
        ...
        # turn into gray
        # print("ihc_d min", ihc_d.min())
        # print("ihc_d max", ihc_d.max())
        ...
        # print("grey_d min", grey_d.min())
        # print("grey_d max", grey_d.max())
        # denominator = grey_d + self.adjust_Calibration
        # denominator = torch.clamp(denominator, min=1e-8)  # 确保分母大于 1e-8
        FOD = torch.log10(1 / grey_d + self.adjust_Calibration)
       
        FOD[FOD<0] = torch.tensor(0.0).cuda()
        # print("FOD min", FOD.min())
        # print("FOD max", FOD.max())
        FOD = FOD**self.alpha
        # Set a threshold to identify and zero out FOD values that are too low
